...
from .utils import overlap_utils, cluster, plot_utils
...
class peakMerger:
    ...
    def pseudoHC(self, annotationFile=None, metric="dice", kMetaSamples=50000, method="ward", figureFmt="pdf"):
        orderRows = cluster.threeStagesHC(self.matrix.T, metric)
        orderCols = cluster.threeStagesHC(self.matrix, metric)
    ...
    def clusterize(self, transpose, altMatrix=None, metric="auto", r=1.0, k="auto", 
        ...
        if reDo or (self.clustered[int(transpose)] is None):
            if transpose:
                if altMatrix is None:
                    self.clustered[1] = cluster.graphClustering(self.matrix.T, 
                                                                    metric, k, r, method=="SNN",
                                                                    restarts=restarts)
                else:
                    self.clustered[1] = cluster.graphClustering(altMatrix, 
                                                                    metric, k, r, method=="SNN",
                                                                    restarts=restarts)
            else:
                if altMatrix is None:
                    self.clustered[0] = cluster.graphClustering(self.matrix, 
                                                                    metric, k, r, method=="SNN",
                                                                    restarts=restarts)
                else:
                    self.clustered[0] = cluster.graphClustering(altMatrix, 
                                                                    metric, k, r, method=="SNN",
                                                                    restarts=restarts)
